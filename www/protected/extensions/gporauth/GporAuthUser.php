...
class GporAuthUser extends CApplicationComponent implements IWebUser
{
    ...
    /**
     * @var IGporAuthDbDriver class which stores user data and tokens.
     */
    ...
    public function login($identity, $rememberMe=false)
    {
        ...
        if (!$data)
        {
            ...
            if ($id)
            {
                ...
                $notifyData = array();
                $notifyData['email'] = isset($_info['email']) ? $_info['email'] : null;
                $notifyData['user_pub_comment_news'] = 1;
                $this->_dbDriver->setUserCustomAttributes($notifyData, $id);
            }
        }
        else
        {
    ...
    public function logout($destroySession=true)
    {
        if($this->beforeLogout())
        {
            if($this->allowAutoLogin)
            {
                Yii::app()->request->getCookies()->remove($this->getStateKeyPrefix());
    ...
    {
        $cookie = Yii::app()->request->getCookies()->itemAt($this->getStateKeyPrefix());

        if($cookie && !empty($cookie->value))
        {
            $data = $this->_dbDriver->findByToken($cookie->value);

            if(is_array($data) && isset($data['id'],$data['duration']))
            {
    ...
    protected function generateToken()
    {
        if ($this->__token === null)
        {
            ...
            $token = md5(serialize($data).time().rand(0, 1000000));
            $this->__token = $token;
        }
        return $this->__token;
    }
    ...
    {
        $token = md5(Yii::app()->params['token_secret'].time().rand(0, 1000000));
        $realToken = $this->getToken();
        $this->_dbDriver->addTemporaryToken($realToken, $token);
        return $token;
    }

    public function findTokenByTemporaryToken($ttoken)
    {
        return $this->_dbDriver->findTokenByTemporaryToken($ttoken);
    }
