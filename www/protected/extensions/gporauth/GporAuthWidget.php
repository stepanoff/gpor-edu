...
class GporAuthWidget extends CWidget {
    ...
    public $providerUse = ''; // authenticate throw this service now
    public $width = false; // width of widget in pixels, default 600
    public $height = false; // height of widget in pixels, default 400
    public $noClose = false; // close window after success authorization?
    ...
    protected function registerAssets()
    {
        ...
        // Open the authorization dialog in popup window.
        if ($this->popup) {
            $js = '';
            foreach ($this->services as $name => $service) {
                ...
                if ($this->providerUse == $name)
                    $js.="$('.auth-service-".$service->getServiceName()." a.auth-link').click();\n";
            }
